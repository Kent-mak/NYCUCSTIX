...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const handleButtonClick = () => {
    console.log(`Navigating to /event/${title}`);
    navigate(`/event/${title}`);
  };

  return (
    <article className="flex flex-col w-full max-w-xs">
      <section className="flex flex-col grow text-xl leading-8 text-black">
        <figure className="overflow-hidden relative flex-col justify-center items-start px-4 pt-4 pb-36 w-full text-3xl font-semibold tracking-tighter leading-8 text-white whitespace-nowrap rounded-lg aspect-square">
        ...
        <button
          ...
          id={title}
          className="justify-center px-4 py-1.5 mt-3 font-bold whitespace-nowrap bg-yellow-500 rounded-lg"
        >
          {buttonText}
        </button>
      </section>
